    ...
    class OnPolicyAlgText(alg_class, OnPolicyWarmStartMixin):
        ...
        def generate_batch(
            ...
            for actions_tensor, _, action_mask in zip(
                ...
                for env_ix in range(self.env.num_envs):
                    ...
                    if actions[env_ix] == tokenizer.eos_token_id and not dones[env_ix]:
                        # generate text using the model
                        obs_tensor = obs_as_tensor(current_obs, self.device)
                        unpacked_obs = unpack_observations(obs_tensor, self.env.num_envs)
                        retrieval_current_obs = unpacked_obs[env_ix]
                        retrieval_tensor = obs_as_tensor(retrieval_current_obs, self.device)
                        

                        retrieval_generation_inputs = self.policy.get_inputs_for_generation(retrieval_tensor)
                        retrieval_gen_output = self.policy.generate(
                            input_ids=retrieval_generation_inputs.inputs,
                            attention_mask=retrieval_generation_inputs.attention_masks,
                            tokenizer=tokenizer,
                        )

                        # process them one step at a time to collect rollout info

                        # here is to be replace 
                        # episode_wise_transitions = [[] for _ in range(self.env.num_envs)]
                        # ep_terminated = np.zeros((self.env.num_envs,), dtype=bool)

                        retrieval_value_past_state = None
                        retrieval_ref_past_state = None
                        retrieval_policy_past_state = None
                        
                        retrieval_masks = (
                            retrieval_gen_output.action_masks
                            if retrieval_gen_output.action_masks is not None
                            else [None] * len(retrieval_gen_output.step_wise_logprobs)
                        )

                        for retrieval_actions_tensor, _, retrieval_action_mask in zip(
                            retrieval_gen_output.step_wise_actions, retrieval_gen_output.step_wise_logprobs, retrieval_masks
                        ):
                            # if all episodes are done, just break and do not continue
                            if ep_terminated[env_ix]:
                                break
                            ...
                            with torch.no_grad():
                                ...
                                # get log probs (TBD: generalize this a bit)
                                retrieval_policy_kwargs = self.get_policy_kwargs(
                                    retrieval_tensor, retrieval_actions_tensor, retrieval_policy_past_state, retrieval_action_mask
                                )

                                retrieval_policy_outputs: PolicyOutput = self.policy.forward_policy(
                                    **retrieval_policy_kwargs
                                )
                                retrieval_raw_log_probs, retrieval_log_probs, retrieval_policy_past_state = (
                                    retrieval_policy_outputs.raw_log_probs,
                                    retrieval_policy_outputs.log_probs,
                                    retrieval_policy_outputs.past_model_kwargs,
                                )

                                # sanity check
                                assert torch.all(
                                    torch.isfinite(retrieval_log_probs)
                                ), "Infinite values in log probs"

                                # sanity check
                                assert torch.all(
                                    torch.isfinite(retrieval_raw_log_probs)
                                ), "Infinite values in log probs"

                                # get values
                                retrieval_value_outputs: ValueOutput = self.policy.forward_value(
                                    retrieval_tensor, retrieval_value_past_state
                                )
                                retrieval_values, retrieval_value_past_state = (
                                    retrieval_value_outputs.values,
                                    retrieval_value_outputs.past_model_kwargs,
                                )

                                # get reference log probs
                                retrieval_ref_policy_outputs: RefPolicyOutput = (
                                    self.policy.get_log_probs_ref_model(
                                       retrieval_tensor, retrieval_actions_tensor, retrieval_ref_past_state
                                    )
                                )
                                retrieval_ref_log_probs, retrieval_ref_past_state = (
                                    retrieval_ref_policy_outputs.log_probs,
                                    retrieval_ref_policy_outputs.past_model_kwargs,
                                )

                                # sanity check
                                assert torch.all(
                                    torch.isfinite(retrieval_ref_log_probs)
                                ), "Infinite values in log probs"

                                retrieval_ref_log_probs = retrieval_ref_log_probs.to(self.device)
                                # compute KL rewards
                                retrieval_kl_div = retrieval_raw_log_probs - retrieval_ref_log_probs
                                retrieval_kl_rewards = -1 * self._kl_controller.kl_coeff * retrieval_kl_div

                            # step into env to get rewards
                            retrieval_actions = retrieval_actions_tensor.cpu().numpy()

                            # step single env
                            # new_obs, rewards, dones, infos = self.env.step(actions)

                            # retrieval_new_obs, retrieval_rews, retrieval_dones, retrieval_infos = self.env[env_ix].step(retrieval_actions)

                            self.env.remotes[env_ix].send(("step", int(retrieval_actions)))
                            self.waiting = True    

                            results = self.env.remotes[env_ix].recv()
                            self.waiting = False
                            obs, retrieval_rews, retrieval_dones, retrieval_infos = results
                            retrieval_new_obs =  _flatten_obs([obs], self.env.observation_space)
                            retrieval_rews = np.array(retrieval_rews)
                            retrieval_dones = np.array(retrieval_dones)
                            
                            # 保存原来的state
                            # original_states = [self.env.env_method('get_state', indices=i) for i in range(self.env.num_envs)]
                            # # 步进第一个环境
                            # retrieval_new_obs, retrieval_rews, retrieval_dones, retrieval_infos = self.env.env_method('step', retrieval_actions, indices=env_ix)

                            # # 恢复其他未步进环境的状态
                            # for i in range(self.env.num_envs):
                            #     if i != env_ix:
                            #         vec_env.env_method('set_state', original_states[i], indices=i)

                            self.num_timesteps += 1 # self.env.num_envs

                            # compute total rewards
                            retrieval_total_rewards = retrieval_rews + retrieval_kl_rewards.cpu().numpy()

                            # unpack individual observations
                            unpack_retrieval_new_obs = unpack_observations(retrieval_tensor, 1)

                            if not ep_terminated[env_ix]:
                                transtion = TransitionInfo(
                                    observation=unpack_retrieval_new_obs[0],
                                    action=retrieval_actions,
                                    task_reward=retrieval_rews,
                                    total_reward=retrieval_total_rewards,
                                    kl_div=retrieval_kl_div.cpu().numpy()[0],
                                    episode_start=episode_starts[env_ix],
                                    value=retrieval_values[0].cpu(),
                                    log_prob=retrieval_log_probs[0].cpu(),
                                    done=retrieval_dones,
                                    ref_log_prob=retrieval_ref_log_probs[0].cpu(),
                                    kl_reward=retrieval_kl_rewards.cpu().numpy()[0],
                                    action_mask=retrieval_action_mask.cpu().numpy()
                                    if retrieval_action_mask is not None
                                    else None,
                                    info=retrieval_infos,
                                )
                            ...
                            retrieval_current_obs = retrieval_new_obs
            ...
            for ep_ix, transitions in enumerate(episode_wise_transitions):
                ...
                for transition_ix, transition in enumerate(transitions):
                    ...
                    if rollout_buffer.full and not advantages_computed:
                        ...
                        # print("rollout_buffer.rewards is:", rollout_buffer.rewards)
        ...
        def collect_rollouts(
            ...
            while not rollout_buffer.full:
                ...
                # generate batch of rollouts of retrieval
                # rollout_info = self.generate_batch_retrieval(
                #     rollout_buffer, tokenizer, max_steps, rollout_info
                # )
