...
class CausalLMActorCriticPolicy(LMActorCriticPolicy, ActorCriticWarmStartMixin):
    ...
    def _build_model_heads(self, model_name: str):
        ...
        self._value_model = self._value_model.to(self.value_device)
        self._value_model = self._value_model.base_model.to(self.value_device)

        self._value_head = self._value_head.to(self.value_device)

        self._ref_model = self._ref_model.to(self.ref_device)
        self._ref_model = self._ref_model.base_model.to(self.ref_device)
    ...
    def get_log_probs_ref_model(
        ...
        # self._ref_model.to(input_ids.device) # OOM
        # self._ref_model.to("cuda:1") 
    ...
    def get_policy_first_device(self):
        return (
            self._policy_model.model.first_device # self._policy_model.transformer.first_device
