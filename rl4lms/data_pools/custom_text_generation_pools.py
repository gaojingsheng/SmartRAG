...
class mmluthree(TextGenPool):
    ...
    def prepare(cls, split: str, prompt_suffix: str = "", prompt_prefix: str = "", ifdebug: bool = False):
        ...
        infile = f'/cpfs/user/gaojingsheng/AliA800/RL4LMs/datasets/tasks/mmluthree/{split}.jsonl'
        if infile.split(".")[-1] == 'jsonl':
            lines = open(infile, 'r', encoding='utf8').readlines()
            lines = [json.loads(l) for l in lines] 
        elif infile.split(".")[-1] == 'json':
...
class NarrativeQA(TextGenPool):
    ...
    def prepare(cls, split: str):
        ...
        for ix, (prompt, answers) in enumerate(prompts_and_answers.items()):
            ...
            samples.append(sample)
            # debug decrease sample number
            # if ix == 10:
            #     break
        dp_instance = cls(samples)
        return dp_instance

class NarrativeQARetrieval(TextGenPool):
    @classmethod
    def normalize_text(cls, text, strip: bool):
        # https: // github.com/allenai/unifiedqa/blob/7bf0653c6fb68a51019924fd4c51615155acbebe/tasks.py  # L54-L58
        # print(text)
    ...
    @classmethod
    def prepare(cls, split: str, prompt_suffix: str = "", prompt_prefix: str = ""):
        ...
        # group questions and answers
        prompts_and_answers = defaultdict(list) # 返回空列表而不是error
        for _, (prompt, answer) in split_df.iterrows():
            prompts_and_answers[prompt].append(answer)
        ...
        for ix, (prompt, answers) in enumerate(prompts_and_answers.items()):
            sample = Sample(id=f"{split}_{ix}",
                            prompt_or_input_text=cls.normalize_text(
                                prompt_prefix + prompt + prompt_suffix, False),
                            references=[cls.normalize_text(answer, True) for answer in answers])
            samples.append(sample)
            # debug decrease sample number
            # if ix == 100:
