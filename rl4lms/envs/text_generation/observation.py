...
class Observation:
    ...
    def update(self, action: int, tokenizer: AutoTokenizer) -> "Observation":
        ...
        context_text = tokenizer.decode(
            current_context.flatten()).replace("<pad>","").replace("</s>","")
    ...
    def init_from_sample(cls, sample: Sample,
        ...
        if context_start_token is not None:
            ...
            # 替换 <s> token 为 padding token
            context_outputs['input_ids'][context_outputs['input_ids'] == s_token_id] = pad_token_id

            # 更新 attention mask
            context_outputs['attention_mask'][context_outputs['input_ids'] == pad_token_id] = 0
