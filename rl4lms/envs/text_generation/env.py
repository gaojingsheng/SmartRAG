...
class TextGenEnv(Env):
    def __init__(
        ...
        if self.retrieval_engine == "contriever":
    ...
    def contriever_search(self, topic, num = 1):
        
        old_time = time.time()
        output = self.retriever.search_document(topic, num)
        res_text = []
        for item in output:
            res_text.append(item['title'] + ": " + item['text'])
    ...
    def step(self, action: int) -> Tuple[Dict[str, torch.tensor], int, bool, dict]:
        ...
        if done:
            ...
            if answer_query == "[Answer]":
                ...
                try:
                    if self.retrieval_engine == "contriever":
                        retrieve_text, _ = self.contriever_search(remaining_sentence)
                    else:
                ...
                #         prompt_or_input_text=self.prompt_input_retrieval.format(input=self.matchv3(self.__current_obs.prompt_or_input_text), retrieval=retrieve_text[:400]),
