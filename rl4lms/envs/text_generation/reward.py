...
def exact_match_list(prediction, ground_truths):
    return max([exact_match_score(prediction, gt) for gt in ground_truths])
    
def f1_list(prediction, ground_truths):
...
class ExactMatchFunction(RewardFunction):
    def __init__(
        self, shaping_fn: str = None, use_single_ref: bool = True, think: bool = True
    ) -> None:
        super().__init__()
        from rl4lms.envs.text_generation.registry import RewardFunctionRegistry
    ...
    def __call__(
        ...
        if done:
            ...
            if self._use_single_ref:
                ...
                references = [next_observation.target_or_reference_texts]
            predicted = [next_observation.context_text]
            # retrieval = next_observation.retrieval
            # retrieval_gt = next_observation.retrieval_gt
            metric_results_f1 = []
            metric_results_em = []
            for i, (p, r) in enumerate(zip(predicted, references)):
                metric_results_em.append(exact_match_list(p, r))
                metric_results_f1.append(f1_list(p, r))

            reward = sum(metric_results_em)/len(metric_results_em) + sum(metric_results_f1)/len(metric_results_f1)
            ...
            # if reward < 0.2 and retrieval:
            #     reward -= 0.5
                
            # elif reward > 1.5 and not retrieval:
            #     reward += 0.5
            # else:
            #     pass
...
class ExactMatchFunctionSingle(RewardFunction):
    def __init__(
        self, shaping_fn: str = None, use_single_ref: bool = True, think: bool = True
    ) -> None:
        super().__init__()
    ...
    def __call__(
        ...
        if done:
            ...
            reward = sum(metric_results_em)/len(metric_results_em) 
...
if __name__ == "__main__":
    predictions = ['Congressman Paul Ryan, Wisconsin']
    references = [['Congressman Paul Ryan from Wisconsin']]
